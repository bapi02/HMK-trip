...
- **Leaflet + CartoDB Voyager 타일** (지도, API 키 불필요). 핀에 한국어 장소명을 상시 표시하고 UI는 전부 한국어.
- **SortableJS** (드래그 재배치)
- **Firebase Hosting** 배포

## 주요 기능

- 여행 보관함: 카드 그리드, 새로 만들기(출발일·도착일 선택) / **복제(날짜만 비움)** / 삭제
- 항공권: 가는 편/오는 편 각각 항공사·편명·공항·출발/도착 일시·비용 입력 → 상세 상단에 요약, 총비용에 합산
